...
const ProductsListing = ({ data, isLoading, isError, HandleEdit, HandleDelete }: Props) => {
  ...
  return (
    <div className="max-w-full lg:max-w-6xl border mx-auto p-2 sm:p-6 md:p-8 mt-10 bg-white rounded-md shadow-md">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl font-bold text-gray-700">Products</h1>
        <button
        ...
        <table className="w-full bg-white table-auto  border-gray-300 rounded-lg">
          ...
          <tbody>
            {data?.data?.slice().reverse().map((product) => (
              <tr key={product._id} className="hover:bg-gray-50 transition duration-100 text-center">
